...
export default function AdminPaymentPage() {
  ...
  const fetchPendingPayments = async () => {
    try {
      const res = await fetch(
        "http://localhost:8000/app/admin/payments/pending/",
        {
          credentials: "include",
        },
      );
  ...
  const handleVerify = async (
    ...
    try {
      ...
      const res = await fetch(
        "http://localhost:8000/app/admin/payments/verify/",
        {
          method: "POST",
          headers: {
      ...
      {alertConfig.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity">
          <div className="bg-white rounded-lg shadow-2xl p-8 max-w-sm w-full text-center transform transition-all">
            ...
            {alertConfig.text && (
              <p className="text-gray-500 mb-6">{alertConfig.text}</p>
            )}
              ...
              >
                <div className="p-5 border-b border-gray-50 bg-gray-50/50">
                  <div className="flex justify-between items-start">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${p.paymenttype === "subscription" ? "bg-purple-100 text-purple-700" : "bg-green-100 text-green-700"}`}
                    >
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-gray-700">
                      {new Date(p.transdate).toLocaleString("th-TH")}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
